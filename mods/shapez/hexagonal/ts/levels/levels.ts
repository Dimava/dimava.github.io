...
export class SzLevel {
	...
	constructor(index: number, required: number, shape: szShapeHash6 | keyof typeof SzInfo.quad.named, reward: enumHubGoalRewards) {
		this.index = index;
		...
		this.shape = shape as szShapeHash6;
	...
	static modifyLevelDefinitions(levels: {
		...
		levels.map(d => {
			if (named[d.shape as keyof typeof SzInfo.quad.named]) {
				d.shape = named[d.shape as keyof typeof SzInfo.quad.named];
			}
	...
	static modifyUpgrades(
		...
		Object.values(upgrades).flat().flatMap(e => e.required)
			.map(e => {
				...
				if (named[e.shape as keyof typeof named]) e.shape = named[e.shape as keyof typeof named];
	...
	static computeFreeplayShape(level: number, rng: RandomNumberGenerator) {
		let layerCount = 1;
