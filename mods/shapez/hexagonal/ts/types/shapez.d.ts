...
declare module "shapez/game/items/shape_item" {
	export abstract class ShapeItem extends BaseItem {
...
declare module "shapez/mods/mod_interface" {
	...
	export class ModInterface {
		...
		registerNewBuilding({
			...
			metaClass: NonAbstract<typeof ModMetaBuilding>;
		 ...
		 */
		extendObject<T>(prototype: T, extender: ({ $super, $old }: { $super: T; $old: T }) => Partial<T>): void;
		/**
		 *
		 * @param {Class} classHandle
		 * @param {({ $super, $old }) => any} extender
		 */
		extendClass<C extends {}, T extends InstanceType<C>>(
			classHandle: C,
			extender: ({ $super, $old }: { $super: T; $old: T }) => Partial<T>
		): void;
		/**
		 *
		 * @param {string} id
