...
// MERGED
/**
 * @typedef {{supply:any,generation:any,upgrade_supply_limit_stage:any,upgrade_effectiveness_stage:any,upgrade_effectiveness_level:any,upgrade_supply_limit_price:any,upgrade_effectiveness_price:any,upgrade_generation_price:any,interval:any,price:any,multiplier:any,init_multiplier:any,recent_money:any,charge:any,audio_mute:any,pb_width:any,x:any,ratios_flag:any,button:any,supply_label:any,tab:any,tab_contents:any,upgrade_supply_limit:any,pb_upgrade_supply_limit:any,upgrade_effectiveness:any,pb_upgrade_effectiveness:any,ratio_label:any,effectiveness_label:any,generation_label:any,upgrade_generation:any,name_label:any,pb_effectiveness_indicator:any,pb_supply_indicator:any,audio_toggle:any,price_label:any,sup_label:any,buymax:any,generation_block:any,effectiveness_block:any}} Merged
 */
/**
 * @returns {Merged}
 */
function makeMerged(number) {
	let o = {};
	const keys = [
		"one_supply",
		"one_generation",
		"one_upgrade_supply_limit_stage",
		"one_upgrade_effectiveness_stage",
		"one_upgrade_effectiveness_level",
		"one_upgrade_supply_limit_price",
		"one_upgrade_effectiveness_price",
		"one_upgrade_generation_price",
		"one_interval",
		"one_price",
		"one_multiplier",
		"one_init_multiplier",
		...
		"one_tab",
		"one_tab_contents",
		"one_upgrade_supply_limit",
		"pb_one_upgrade_supply_limit",
		"one_upgrade_effectiveness",
		"pb_one_upgrade_effectiveness",
		"one_ratio_label",
		"one_effectiveness_label",
		"one_generation_label",
		"one_upgrade_generation",
		"one_name_label",
		"pb_one_effectiveness_indicator",
		"pb_one_supply_indicator",
		"audio_toggle_one",
		"one_price_label",
		"sup_one_label",
		"one_buymax",
		"one_generation_block",
		"one_effectiveness_block",
	];
	for (let key of keys) {
		let pureKey = key.replace(/(\b|_)one(\b|_)/, '_').replace(/^_|_$|_(?=_)/, '');
		let numberKey = key.replace(/(?<=_|\b)one(?=_\w+|\b)/, number);
		Object.defineProperty(o, pureKey, {
			get() {
				return window[numberKey];
			},
			set(v) {
				window[numberKey] = v;
			},
...
const one_ = makeMerged('one');
const two_ = makeMerged('two');
const three_ = makeMerged('three');
const four_ = makeMerged('four');
...
$(document).ready(function () {
	...
	//GENERATORS
	/** @param {Merged} glob */
	function make_upgrade_supply_limit_click(glob) {
		glob.upgrade_supply_limit.click(function () {
			...
			if (glob.price == 0) { button1Enable(glob.button); }

			while (money - glob.upgrade_supply_limit_price >= 0 && cycles > 0) {
				money -= glob.upgrade_supply_limit_price;

				glob.upgrade_supply_limit_stage += 20;
				if (glob.upgrade_supply_limit_stage == 100) { glob.upgrade_supply_limit_stage = 0; glob.price = glob.price * 2; }
				else { glob.price += supply_base; }

				glob.upgrade_supply_limit_price = glob.upgrade_supply_limit_price + glob.upgrade_supply_limit_price * sgr;
				cycles--;
			}

			if (glob.upgrade_supply_limit_stage == 80) { label = "x2"; }
			else { label = "+" + supply_base; }

			glob.button.text(numT(glob.price));

			glob.upgrade_supply_limit.text("⌬" + numT(glob.upgrade_supply_limit_price));

			progress3(glob.upgrade_supply_limit_stage, glob.pb_upgrade_supply_limit, glob.pb_supply_indicator, label);
	...
	/** @param {Merged} glob */
	function make_upgrade_effectiveness_click(glob) {

		glob.upgrade_effectiveness.click(function () {
			...
			while (money - glob.upgrade_effectiveness_price >= 0 && cycles > 0) {

				glob.ratios_flag = 1;

				money -= glob.upgrade_effectiveness_price;

				glob.upgrade_effectiveness_stage += 4;
				if (glob.upgrade_effectiveness_stage == 100) {

					if (glob.upgrade_effectiveness_level % 2 === 0) { glob.upgrade_effectiveness_stage = 0; glob.multiplier = glob.multiplier * 100; }
					else { glob.upgrade_effectiveness_stage = 0; glob.multiplier = glob.multiplier * 5; }

					glob.upgrade_effectiveness_level++;

				}
				else { glob.multiplier += glob.init_multiplier; }

				glob.upgrade_effectiveness_price = glob.upgrade_effectiveness_price + glob.upgrade_effectiveness_price * egr;

				//check if the next iteration puts the price over the next generation price and stop buymax
				if (glob.upgrade_effectiveness_price + glob.upgrade_effectiveness_price * egr > glob.upgrade_generation_price) {
					glob.upgrade_effectiveness.css("visibility", "hidden").blur();
					cycles = 0;

					if (glob.generation >= generation_limit) { glob.effectiveness_block.hide(); }
				}

				cycles--;
			}

			if (glob.upgrade_effectiveness_level % 2 === 0) { glob.sup_label.text("x100"); }
			else { glob.sup_label.text("x5"); }

			if (glob.upgrade_effectiveness_stage == 96) {//multiplier labels
				if (glob.upgrade_effectiveness_level % 2 === 0) { label = "x100"; }
				else { label = "x5"; }
			}
			else { label = "+" + numT(glob.init_multiplier); }

			glob.upgrade_effectiveness.text("⌬" + numT(glob.upgrade_effectiveness_price));


			progress3(glob.upgrade_effectiveness_stage, glob.pb_upgrade_effectiveness, glob.pb_effectiveness_indicator, label);

			glob.effectiveness_label.text("[" + numT(glob.multiplier) + "]");
	...
	/** @param {Merged} glob */
	function make_upgrade_generation_click(glob, fixed_multiplier_list) {

		glob.upgrade_generation.click(function () {

			if (money < glob.upgrade_generation_price) return;
			...
			glob.generation++;
			if (glob.generation == generation_limit) { glob.generation_block.hide(); }
	
			//to make sure we will recalculate ratios when we start this generator
			glob.ratios_flag = 1;
	
			money -= glob.upgrade_generation_price;
	
			//generating generator multiplier
			glob.init_multiplier = glob.init_multiplier * 100000000;
	
			var fixed_multiplier;
	
			fixed_multiplier = fixed_multiplier_list[glob.generation];

			glob.multiplier = prestige_multiplier * fixed_multiplier;
	
	
			//effectiveness price
			glob.upgrade_effectiveness_price = Math.floor(glob.upgrade_generation_price);
			glob.upgrade_effectiveness.text("⌬" + numT(glob.upgrade_effectiveness_price));
			//supply limit price
			glob.upgrade_supply_limit_price = Math.floor(glob.upgrade_generation_price);
			glob.upgrade_supply_limit.text("⌬" + numT(glob.upgrade_supply_limit_price));
	
			//resetting the generator
			glob.x = 0; g_plasma.css('background-position', + glob.x + 'px 0px');
			clearInterval(glob.interval);
	
			//resetting effectiveness
			glob.upgrade_effectiveness_stage = 0;
			glob.upgrade_effectiveness_level = 1; glob.sup_label.text("x5");
	
			glob.effectiveness_label.text("[" + numT(glob.multiplier) + "]");
			label = "+" + numT(glob.init_multiplier);
			progress3(glob.upgrade_effectiveness_stage, glob.pb_upgrade_effectiveness, glob.pb_effectiveness_indicator, label);
	
			//resetting supply
			glob.upgrade_supply_limit_stage = 0;
			glob.supply = 0; glob.supply_label.text(glob.supply);
			glob.price = supply_base;
			glob.button.text(numT(glob.price));
			button1Enable(glob.button);
	
			progress3(0, glob.pb_upgrade_supply_limit, glob.pb_supply_indicator, "+" + supply_base);
	
			//generator generation price
			glob.upgrade_generation_price = glob.upgrade_generation_price * Math.pow(10, 14);
			glob.upgrade_generation.text("⌬" + numT(glob.upgrade_generation_price));
	
			//other updates
			glob.generation_label.text("Generation " + romanize(glob.generation + 1));
			glob.name_label.text("Plasma " + romanize(glob.generation));
			glob.upgrade_effectiveness.css("visibility", "visible");
			InventoryUpdate();
	
		});
	}

	make_upgrade_supply_limit_click(one_);
	make_upgrade_effectiveness_click(one_);
	make_upgrade_generation_click(one_, [
		0, 1,
		746441400,
		454026565000000000,
		9.2825459625e+25,
		1.5210396353124983e+34,
		2.2620209341406282e+42,
		3.188247557675796e+50,
		4.346030837094735e+58,
		5.793259936368435e+66,
		7.602296310460565e+74,
	]);

	make_upgrade_supply_limit_click(two_);
	make_upgrade_effectiveness_click(two_);
	make_upgrade_generation_click(two_, [
		0, 1,
		74644170000,
		45402660250000000000,
		9.282546431250019e+27,
		1.5210396939062585e+36,
		2.262021007382825e+44,
		3.188247649228516e+52,
		4.3460309515356394e+60,
		5.793260079419542e+68,
		7.602296489274422e+76,
	]);

	make_upgrade_supply_limit_click(three_);
	make_upgrade_effectiveness_click(three_);
	make_upgrade_generation_click(three_, [
		0, 1,
		7464420000000,
		4.540266399999992e+21,
		9.282546899999991e+29,
		1.521039752499998e+38,
		2.262021080625002e+46,
		3.1882477407812464e+54,
		4.346031065976573e+62,
		5.793260222470728e+70,
		7.602296668088394e+78,
	]);

	make_upgrade_supply_limit_click(four_);
	make_upgrade_effectiveness_click(four_);
	make_upgrade_generation_click(four_, [
		0, 1,
		746442300000000,
		4.540266774999995e+23,
		9.282547368750024e+31,
		1.521039811093754e+40,
		2.262021153867188e+48,
		3.188247832333969e+56,
		4.3460311804174426e+64,
		5.793260365521837e+72,
		7.602296846902273e+80,
	]);
